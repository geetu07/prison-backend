...
app.post("/prisoners/add", (req, res) => {
    const { Name, DOB, Gender, Address, Sentence_Duration, Supervisor_ID } = req.body;  // No Prisoner_ID

    if (!Name || !DOB || !Gender || !Address || !Sentence_Duration || !Supervisor_ID) {
        return res.status(400).json({ message: "⚠️ All fields are required!" });
    }

    const sql = `
        INSERT INTO prisoner (Name, DOB, Gender, Address, Sentence_Duration,Supervisor_ID) 
        VALUES (?, ?, ?, ?, ?,?)
    `;

    db.query(sql, [Name, DOB, Gender, Address, Sentence_Duration,Supervisor_ID], (err, result) => {
        if (err) {
